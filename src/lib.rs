...
pub extern "C" fn xCheckpoint(
    ...
    let frame_count = if frames_in_wal.is_null() {
        0
    } else {
        unsafe { *frames_in_wal }
    };
    let backfilled = if backfilled_frames.is_null() {
        0
    } else {
        unsafe { *backfilled_frames }
    };
    tracing::info!("Frames: {}; backfilled: {}", frame_count, backfilled);
